...
import { Button, Icon, useDisclosure, Modal, ModalOverlay, ModalContent, ModalBody, ModalCloseButton, Stack, Heading, Text, Flex } from "@chakra-ui/react";
...
export const AddToCartButton = ({ ProductId, quantity, name, isText = false, ml = 0 }) => {
    ...
    const handleAdd = async() => {
        try {
            const { data } = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/cart`, { ProductId, quantity, BranchId }, {
                headers: {
    ...
    const handleAbandon = async() => {
        try {
            await axios.put(`${process.env.REACT_APP_API_BASE_URL}/cart`, { BranchId }, {
                headers: {
                    authorization: `Bearer ${token}`
                }
            });
