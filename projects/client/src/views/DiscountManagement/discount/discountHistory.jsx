	...
	const fetchData = async () => {
		try {
			...
			const { data } = await axios.get(
				...
				}/discount?limit=8&type=${type}&page=${page}&branchId=${branchId}&search=${search}&startAvailableFrom=${startAvailableFrom}&endAvailableFrom=${endAvailableFrom}&startValidUntil=${startValidUntil}&endValidUntil=${endValidUntil}&sortBy=${sortBy}&order=${
					order ? "ASC" : "DESC"
				}`,
				{
					headers: {
							...
							return (
								<Tr key={idx}>
									...
									<Td alignItems={"center"} justifyContent={"center"}>
										{new Date(availableFrom) < new Date(Date.now()) &&
