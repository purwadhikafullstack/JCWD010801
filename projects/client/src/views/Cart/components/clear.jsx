...
export const ClearCart = ({ isEmpty }) => {
	...
	const handleClear = async () => {
		try {
			...
			await axios.patch(
				`${process.env.REACT_APP_API_BASE_URL}/cart/clear`,
				{ description },
				{
					headers: {
					...
					<ModalContent>
						<ModalBody p={5}>
							<Stack p={3} gap={5} w="100%" h="100%" justifyContent={"center"} alignItems={"center"}>
								...
								<Heading>Give us your feedback</Heading>
								<Text textAlign={"center"} fontWeight={"light"}>
									Would you mind giving us some feedback on why you abandon your cart so we can boost your shopping
									experiece. It would be much appreciated.
								</Text>
								<Textarea w={"100%"} maxH={"40px"} ref={feedbackRef} placeholder="Enter your feedback here" />
								<Flex w="100%" gap={5}>
