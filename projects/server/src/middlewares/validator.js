...
module.exports = {
	checkRegister: async (req, res, next) => {
		try {
			...
			await body("firstName").notEmpty().withMessage("First name must not be empty").run(req);
			await body("lastName").notEmpty().withMessage("Last name must not be empty").run(req);
			await body("email")
				.notEmpty()
