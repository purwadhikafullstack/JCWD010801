...
module.exports = {
	createVoucher: async (req, res) => {
		...
		try {
			...
			if (type === "Single item" && !ProductId)
				throw { status: false, message: "Please enter the product id to create single item vouchers" };
		...
		try {
			...
			const filter = {
				where: {
					...
					availableFrom: {
						[Op.between]: [new Date(`${startAvailableFrom}`), new Date(`${endAvailableFrom}`)],
					},
					validUntil: {
						[Op.between]: [new Date(`${startValidUntil}`), new Date(`${endValidUntil}`)],
					},
					BranchId:
						checkRole.RoleId === 3 ? { [Op.or]: { [Op.like]: `%${branchId}%`, [Op.eq]: null } } : checkRole.BranchId,
				},
				limit,
				offset: limit * (page - 1),
